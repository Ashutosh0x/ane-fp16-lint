...
With Apple's AFM 3 Core Advanced deploying a **20-billion-parameter sparse model** on ANE via fp16, numerical safety is no longer an academic concern — it's a production requirement affecting **1 billion+ devices**.
...
$ ane-fp16-lint model.mlpackage
  ...
  ✅ conv2d           — SAFE (linear, bounded by weights)
  ✅ relu             — SAFE (monotone, non-amplifying)
  ❌ softplus         — OVERFLOW RISK: exp(x) overflows at x > 11.09
  ❌ reduce_log_sum_exp — OVERFLOW RISK: sum(exp(x)) overflows at x > 7.63 (C=32)
  ⚠️  softmax         — UNDERFLOW RISK: small probabilities flush to 0
  ❌ log              — DIVERGENCE RISK: log(0) = -inf after softmax underflow
...
| `reduce_log_sum_exp` | 🔴 CRITICAL | x > ln(65504/C) | max(x) + log(Σ exp(x-max(x))) |
| `log_softmax` | 🔴 CRITICAL | softmax underflow | x - max(x) - log(Σ exp(x-max(x))) |
...
1. **Load**: Parse `.mlpackage` → extract MIL (Model Intermediate Language) graph
2. **Walk**: Traverse operations in topological order
3. **Match**: Check each op against the rule database of fp16-unsafe patterns
